import { signupService, loginService, refreshTokenService, logoutService, } from "../services/authService.js";
...
export const signup = async (req, res, next) => {
    try {
        ...
        const { accessToken, refreshToken, user } = await signupService(name, email, mobileNumber, password);
...
export const login = async (req, res, next) => {
    try {
        ...
        const { accessToken, refreshToken, user } = await loginService(emailOrPhone, password);
        res.cookie("refreshToken", refreshToken, {
            httpOnly: true,
            secure: env.NODE_ENV === "production",
            sameSite: env.NODE_ENV === "production" ? "strict" : "lax", // Use 'lax' in development for better compatibility
